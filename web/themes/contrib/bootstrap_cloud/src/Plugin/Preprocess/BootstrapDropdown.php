...
namespace Drupal\bootstrap_cloud\Plugin\Preprocess;
...
use Drupal\Core\Url;
use Drupal\bootstrap\Plugin\Preprocess\BootstrapDropdown as BaseBootstrapDropdown;
 ...
 */
class BootstrapDropdown extends BaseBootstrapDropdown {
   ...
   *
   * Override BootstrapDropdown to keep operation buttons from being btn-xs.
   *
    ...
    if (Unicode::strpos($variables->theme_hook_original, 'links__dropbutton') !== FALSE && !empty($variables->links)) {
      ...
      foreach ($variables->links as &$element) {
        ...
        // Otherwise, convert into an actual "link" render array element.
          ...
          // Find glyphicon for element, but exclude the first one.
          $icon = $i > 0 ? Bootstrap::glyphiconFromString($element['title']) : [];
          $i++;

          // Build render array.
          $element = [
            '#type' => 'link',
            '#title' => $icon ? new FormattableMarkup(Element::create($icon)->renderPlain() . '@text', ['@text' => $element['title']]) : $element['title'],
            '#url' => $element['url'],
