...
class ApiController extends ControllerBase implements ApiControllerInterface {
   ...
   */
  public function getAllNodeAllocatedResources() {
    $response = [];
    ...
    foreach ($nodes ?: [] as $node) {
      if (k8s_is_metrics_enabled($node->getCloudContext()) === TRUE) {
        if ($node->getDirty()) {
          $this->k8sService->setCloudContext($node->getCloudContext());
          $this->k8sService->updateNodesWithoutBatch([
            'metadata.name' => $node->getName(),
          ], FALSE);
        }

        $pod_resources = $this->getPodResources($node->getName());
   ...
   *   The metrics information on node(s).
   */
  public function getNodeAllocatedResources($cloud_context, CloudProjectInterface $cloud_project = NULL, K8sNode $k8s_node = NULL) {
